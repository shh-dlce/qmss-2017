...
# You should have these libraries installed already. If not, use install.packages()
...
setwd("~/Dropbox/Teaching/QMSS/Sessions/Plotting Phylogenies/") #set working directory
...
#Regardless, try playing around with the command 'ggtree.' (Consider ?ggtree if you need more inspiration.)
...
gzoom(PN, unlist(Descendants(PN,300)))
...
ggtree(PN)+geom_cladelabel(node=300, label="exciting", align=TRUE, angle=90, offset=.5,color="red")
